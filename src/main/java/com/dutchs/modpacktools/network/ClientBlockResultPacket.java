...
public class ClientBlockResultPacket implements INetworkPacket {
    ...
    public ClientBlockResultPacket(@NotNull String state, @NotNull BlockPos pos, @NotNull String type, @NotNull String entityType, @NotNull String clazz, @NotNull CompoundTag nbt) {
        blockState = state;
        blockPos = pos;
        blockType = type;
    ...
    public void encode(Object msg, FriendlyByteBuf packetBuffer) {
        ...
        packetBuffer.writeUtf(blockPacket.blockEntityType);
    ...
    public <MESSAGE> MESSAGE decode(FriendlyByteBuf packetBuffer) {
        ...
        result.blockEntityType = packetBuffer.readUtf();
    ...
    public void handle(Object msg, CustomPayloadEvent.Context context) {
        context.enqueueWork(() -> {
            DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> handleClient((ClientBlockResultPacket) msg, context));
        });
        context.setPacketHandled(true);
    }

    private void handleClient(ClientBlockResultPacket msg, CustomPayloadEvent.Context context) {
        ...
        if (!StringUtil.isNullOrEmpty(msg.blockEntityType))
            keyValues.add(new Tuple<>("BlockEntityType", msg.blockEntityType));
