...
//    }
    ...
    public static String createTPCommand(String target, ResourceKey<Level> fromDim, ResourceKey<Level> toDim, BlockPos pos) {
    ...
    public static void SendBlockCommand(boolean nbt){
        ...
        if ((hit != null ? hit.getType() : null) == HitResult.Type.BLOCK) {
            ...
            ModpackTools.NETWORK.toServer(new BlockPacket(pos, false, nbt, false, false));
        } else {
    ...
    public static void SendBlockInvCommand(boolean nbt, boolean remove, boolean json) {
        HitResult hit = Minecraft.getInstance().hitResult;
        if ((hit != null ? hit.getType() : null) == HitResult.Type.BLOCK) {
            BlockPos pos = ((BlockHitResult) hit).getBlockPos();
            ModpackTools.NETWORK.toServer(new BlockPacket(pos, true, nbt, remove, json));
        } else {
            ModpackTools.NETWORK.toServer(new PrivilegedMessagePacket("Not looking at valid block"));
        }
    }
    ...
    public static boolean TakePanorama(int res) {
